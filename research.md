...
<ul>
    <li>
        ...
        <div class="abstract">
            <strong>Abstract:</strong>
            <p>
                The production of large-scale political datasets typically demands extracting structured facts from vast piles of unstructured documents or web sources, a task that traditionally relies on expensive human experts and remains prohibitively difficult to automate at scale. In this paper, we leverage Large Language Models (LLMs) to automate the extraction of multi-dimensional elite biographies, addressing a long-standing bottleneck in political science research. We propose a two-stage ``Synthesis-then-Coding'' agentic framework: an upstream synthesis stage that uses recursive LLM calls to search, filter, and refine evidence from heterogeneous web sources, followed by a downstream coding stage that maps this refined evidence into structured dataframes. We validate this framework through three primary results. First, we demonstrate that when given curated contexts, LLM coders match or outperform human experts in extraction accuracy. Second, we show that our agentic workflow generates biographical corpora with more facts than human collective intelligence (Wikipedia), particularly in fragmented and high-volume information environments. Finally, we diagnosed that extracting from long and multi-language context introduces coding bias that the synthesis stage can alleviate by compressing evidence into signal-dense representations. By distinguishing extraction from classification, we provide a generalizable, scalable solution for converting unstructured narratives into rigorous political data.
            </p>
        </div>
    </li>
